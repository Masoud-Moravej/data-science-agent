...
def return_instructions_analytics() -> str:
  ...
  **Imported Libraries:** The following libraries are ALREADY imported and
  ...
  You can also use the helper function `explore_df(df: pd.DataFrame)` to inspect
  a DataFrame quickly.

  **Matplotlib Usage:** Build plots as usual and call `plt.show()` at the end so
  the figure is captured and returned automatically. Call `plt.close()` only if
  you intentionally want to clear state between plots.
  ...
  **No Assumptions:** **Crucially, avoid making assumptions about the nature of
  ...
  use the information obtained from exploring the data to guide your analysis.
